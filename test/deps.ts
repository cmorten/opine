export { deferred } from "https://deno.land/std@0.113.0/async/deferred.ts";
export type { Deferred } from "https://deno.land/std@0.113.0/async/deferred.ts";
export { expect, mock } from "https://deno.land/x/expect@v0.2.9/mod.ts";
export { superdeno } from "https://deno.land/x/superdeno@4.6.0/mod.ts";
export type {
  IRequest as SuperDenoRequest,
  IResponse as SuperDenoResponse,
} from "https://deno.land/x/superdeno@4.6.0/mod.ts";
